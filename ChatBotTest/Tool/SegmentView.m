...
- (instancetype)initWithFrame:(CGRect)frame andControllers:(NSArray <UIViewController *> *)controllers{
    ...
    if(self){
        ...
        if (controllers.count >=4) {
            self.titleBtnWidth = self.frame.size.width/4;
        }
        else{
            self.titleBtnWidth = self.frame.size.width/controllers.count;
        }
        [self setupState];
        [self setupTitleView];
        [self setupMainView];
...
- (void)setupTitleView{
    _titleScrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, _titleViewHeight)];
...
- (void)setupMainView {
    _mainScrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, _titleViewHeight, self.frame.size.width, self.frame.size.height-_titleViewHeight)];
    _mainScrollView.contentSize = CGSizeMake(_controllers.count*self.frame.size.width, 0);
    ...
    for (int i = 0; i < _controllers.count; i++) {
        ...
        view.frame = CGRectMake(i * self.frame.size.width, 0, self.frame.size.width, self.frame.size.height-_titleViewHeight);
...
- (void)clickBtn:(UIButton *)sender {
    if (sender.tag != _currentIndex) {
        [_mainScrollView setContentOffset:CGPointMake(sender.tag * self.frame.size.width, 0) animated:YES];
    }
}

- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    NSInteger currentIndex = round(_mainScrollView.contentOffset.x / self.frame.size.width);
    if (currentIndex != _currentIndex) {
        _btnArray[_currentIndex].selected = NO;
        _currentIndex = currentIndex;
        _btnArray[_currentIndex].selected = YES;
        [UIView animateWithDuration:0.2f animations:^{
            self->_sliderView.frame = CGRectMake(currentIndex * self->_titleBtnWidth, self->_titleViewHeight - 2, self->_titleBtnWidth, 2);
            if (self->_btnArray[currentIndex].frame.origin.x < self.frame.size.width/2) {
                [self->_titleScrollView setContentOffset:CGPointMake(0, 0) animated:YES];
                //在屏幕中线左边的情况
            } else if (self->_titleScrollView.contentSize.width - self->_btnArray[currentIndex].frame.origin.x <= self.frame.size.width/2) {
                [self->_titleScrollView setContentOffset:CGPointMake(self->_controllers.count*self->_titleBtnWidth-self.frame.size.width, 0) animated:YES];
                //在最右边的情况
            } else {
                [self->_titleScrollView setContentOffset:CGPointMake(self->_btnArray[currentIndex].frame.origin.x-self.frame.size.width/2+self->_titleBtnWidth/2, 0) animated:YES];
                //在中间的情况
            }
        } completion:nil];
